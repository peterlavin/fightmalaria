...
def main():
    ...
    ''' if result set is returned empty, update prot status as 1, wait for next request ''' 
    try:
        if len(result_2) == 0:
            pass
    except:
        JobSelection.update_complete_prot(cursorDB, prot_name)
        sys.exit()
    ...
    success_wu = FileTransfer.boinc_invoke_cmd(cmd_str_1) ###can be  neutered for test
    ...
    success_wu = FileTransfer.boinc_invoke_cmd(cmd_str_2) ###### can be neutered for test 
